...
export default function Accordion({title, header, content}:{
  ...
  return (
    <Disclosure key={`${title}-item`}>
    {({ open }) => (
      /* Use the `open` state to conditionally change the direction of an icon. */
      <div className='px-8 flex flex-col justify-center md:block text-white py-6'>
        <Disclosure.Button className='w-full my-4'>
            <div className='flex justify-between'>
                <div className='uppercase text-lg whitespace-nowrap'>{title}</div>
        ...
        <div className={`${
          open ? `max-h-48 h-fit` : `max-h-0 md:max-h-fit`
            ...
            {content.map( item =>(
              <li key={`${item}-item`} className='list-none py-1'>{item}</li>
          ))}
            </Disclosure.Panel>
        </div>
      </div>
